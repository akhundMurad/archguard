...
def test_nodes_grouped_by_kind_container_layer_context_sorted_deterministically():
    ...
    assert idx.nodes_by_container["c1"] == tuple(sorted([n1, n2], key=lambda x: (x.kind.value, str(x.id), x.path or "", x.name or "")))
    assert idx.nodes_by_layer["domain"] == tuple(sorted([n1, n2], key=lambda x: (x.kind.value, str(x.id), x.path or "", x.name or "")))
    assert idx.nodes_by_context["ctx"] == tuple(sorted([n1, n2], key=lambda x: (x.kind.value, str(x.id), x.path or "", x.name or "")))
...
def test_edges_sorted_deterministically_by_type_src_dst_and_location():
    ...
    expected = sorted([e1, e2, e3, e4], key=lambda e: (
        e.dep_type.value,
        str(e.src),
        str(e.dst),
