...
from archguard.rules.errors import RulesCompileError
from archguard.rules.types import RuleAction, RuleTarget
...
def test_except_when_target_mismatch_is_compile_error():
    r = RuleAst(
        ...
        except_when=(
            NodeWhenAst(
                predicate=CompareAst(left=FieldAst(path="layer"), op="==", right=lit_str("domain"))
            ),
        ),
    )
...
def test_not_or_and_expression_compilation():
    r = RuleAst(
        ...
        when=DependencyWhenAst(
            predicate=OrAst(items=(
                CompareAst(left=FieldAst(path="dep.type"), op="==", right=lit_str("import")),
                NotAst(
                    item=CompareAst(left=FieldAst(path="to.layer"), op="==", right=lit_str("infra"))
                ),
            ))
        ),
    )
